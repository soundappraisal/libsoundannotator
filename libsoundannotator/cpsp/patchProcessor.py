...
class Patch(object):
    ...
    def __init__(self,level,t_low, t_high, s_low, s_high, size, t_offset=None, typelabel=None, serial_number=0, samplerate=0, chunknumber=None,sourcedescriptor=None, **kwargs):
...
class patchProcessorCore(object):
    ...
    def processData(self, chunk,sourcedescriptor='Unknown source'):
        ...
        if not chunk.data.shape[1]==0:
            ...
            self.newPatches(chunk.data, chunk.initialSampleTime, chunk.number,sourcedescriptor)
    ...
    def newPatches(self,data,initialSampleTime,chunknumber,sourcedescriptor):
            '''
...
class patchProcessor(Processor):
    ...
    def processData(self,compositeChunk ):
        chunk=compositeChunk.received['TSRep']
        sourcedescriptor=self.getSourceDescriptor(self.config, compositeChunk.metadata)
        dataout=self.patchProcessorCore.processData(chunk,sourcedescriptor=sourcedescriptor)
    ...
    def getSourceDescriptor(self, config, metadata):
        sourcedescriptor='Unknown source'
        
        if 'usesource_id' in config.keys() and config['usesource_id'] is True:
            sourcedescriptor=metadata[config['source_processor']][2]
